...
300 words
   ...
   of writing on a device. They are polling and interrupts. Interrupts are a hardware device driven
   mechanism whereby the processors tells the device to notify it when it requires attention. E.g. a
   packet arriving on the network card. The processor stops what its doing to service the interrupt.
   Polling is a protocol where a program periodically checks whether the device is ready to receive
   data or not. When the frequency of an interrupt is low, it is more efficient to use an interrupt
   mechanism so that the CPU will not waste cycles periodically polling the device for no reason.
   However, when the interrupt frequency is high, such as in a high performance operation, the
   overhead generated by a large number of interrups ends up being less efficient that polling. So
   it is better in high performance operations to use polling rather than interrupts. 

   Since the experiment described in this report is a high performance operation it wo

100 more needed (as DMA was longer)
